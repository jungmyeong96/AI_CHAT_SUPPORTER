...
* ⚠️ AGENT 필수 자동 구동 규칙: 에이전트는 인프라 구성 시 사용자가 별도로 모델을 다운로드하는 번거로움을 없애기 위해, 컨테이너 기동 즉시 llama3.2:3b와 nomic-embed-text 모델을 자동으로 내부 저장소에 pull(다운로드)하도록 entrypoint 스크립트를 반드시 설계 단계에서 포함시켜야 합니다.
